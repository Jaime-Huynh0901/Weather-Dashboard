...
function getWeather(cityName) {
    ...
    const apiUrl = `https://crossorigin.me/http://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${apiKey}`;
    ...
    .then ( response => {
        ...
        const oneCallUrl = `https://crossorigin.me/http://api.openweathermap.org/data/2.5/onecall?lat=${response.coord.lat}&lon=${response.coord.lon}&exclude=hourly,minutely&appid=${apiKey}`;
...
function generateHTML (data, cityName) {
    ...
    $('#currentWeather').html(`
    <h2>${cityName} (${currentDate}) <span><img src='https://crossorigin.me/http://openweathermap.org/img/wn/${data.current.weather[0].icon}.png' alt ='Weather Icon'></span></h2>
    ...
    for (let i = 1 ; i <=5; i++){
        ...
        $(`#title${[i]}`).html(`
        <h5>${dailyDate}</h5>
        <img src='https://crossorigin.me/http://openweathermap.org/img/wn/${data.daily[i].weather[0].icon}.png' alt ='Weather Icon'>
